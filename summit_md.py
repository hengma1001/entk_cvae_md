...
def generate_MD_stage(num_MD=1): 
    ...
    for i in range(num_MD):
        ...
        if initial_MD: 
            t1.arguments += ['--length', settings['LEN_initial']] 
        else: 
            t1.arguments += ['--length', settings['LEN_iter']]
...
def generate_interfacing_stage(): 
    ...
    stage.post_exec = {
                      'condition': lambda : settings['CUR_STAGE'] < settings['MAX_STAGE'],
                      'on_true': fill_pipeline,
...
def fill_pipeline(p, settings): 
    if settings['CUR_STAGE']:
        print('finishing stage %d of %d' % (settings['CUR_STAGE'], settings['MAX_STAGE'])) 
    ...
    if settings['CUR_STAGE'] % settings['RETRAIN_FREQ'] == 0: 
        # --------------------------
        # Aggregate stage
    ...
    s4 = generate_interfacing_stage(settings) 
    p.add_stages(s4) 
    
    settings['CUR_STAGE'] += 1


def init_pipeline(settings):
    """
    Parameters
    ----------
    settings : dict
        stores pipeline settings
...
if __name__ == '__main__':
    ...
    # TODO: Do we always want to start the current stage at 0?
    #       CUR_STAGE shouldn't be part of settings.

    settings = {
               'CUR_STAGE': 0,     # Current stage
               'MAX_STAGE': 1,     # Number of stages to run
               'RETRAIN_FREQ': 5,  # Frequency to update CVAE weights
               'LEN_iter': 10,     # Time to run initial simulation
               'LEN_initial': 10   # Time to run iterative simulations
               }
    ...
    appman.resource_desc = resources

    p1 = init_pipeline(settings)
